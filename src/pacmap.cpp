...
// ---------- gradient (serial: writes to grad[i]/grad[j] would race if parallelized) ----------

static double pacmap_grad_serial(const NumericMatrix& Y,
  ...
  // NN attractive term: L = w_nb * d/(10+d), grad_i = w_nb * 20/(10+d)^2 * (y_i - y_j)
  int nnp = pair_nb.nrow();
  for (int t = 0; t < nnp; ++t) {
    int i = pair_nb(t, 0), j = pair_nb(t, 1);
    double d_ij = 1.0;
  ...
  // MN attractive term: scaled by 10000
  int nmn = pair_MN.nrow();
  for (int t = 0; t < nmn; ++t) {
    int i = pair_MN(t, 0), j = pair_MN(t, 1);
    double d_ij = 1.0;
    for (int d = 0; d < dim; ++d) { yij[d] = Y(i, d) - Y(j, d); d_ij += yij[d] * yij[d]; }
    loss += w_mn * (d_ij / (10000.0 + d_ij));
    double w1 = w_mn * (20000.0 / std::pow(10000.0 + d_ij, 2));
    for (int d = 0; d < dim; ++d) {
      grad(i, d) += w1 * yij[d];
      grad(j, d) -= w1 * yij[d];
    }
  }
  // FP repulsive term: L = w_fp / (1+d), grad_i = -w_fp * 2/(1+d)^2 * (y_i - y_j)
  int nfp = pair_FP.nrow();
  for (int t = 0; t < nfp; ++t) {
    int i = pair_FP(t, 0), j = pair_FP(t, 1);
    double d_ij = 1.0;
    for (int d = 0; d < dim; ++d) { yij[d] = Y(i, d) - Y(j, d); d_ij += yij[d] * yij[d]; }
    loss += w_fp * (1.0 / (1.0 + d_ij));
    double w1 = w_fp * (2.0 / std::pow(1.0 + d_ij, 2));
    for (int d = 0; d < dim; ++d) {
      grad(i, d) -= w1 * yij[d];
      grad(j, d) += w1 * yij[d];
    }
...
static double pacmap_grad_fit_serial(const NumericMatrix& Y,
  ...
  int npairs = pair_XP.nrow();
  for (int t = 0; t < npairs; ++t) {
    int i = pair_XP(t, 0), j = pair_XP(t, 1);
    if (i < n_basis) continue;  // safety: only new points move
    double d_ij = 1.0;
    for (int d = 0; d < dim; ++d) { yij[d] = Y(i, d) - Y(j, d); d_ij += yij[d] * yij[d]; }
    loss += w_nb * (d_ij / (10.0 + d_ij));
    double w1 = w_nb * (20.0 / std::pow(10.0 + d_ij, 2));
    for (int d = 0; d < dim; ++d) grad(i, d) += w1 * yij[d];
    // NOTE: no grad update for j -- basis points are frozen.
  }
  return loss;
}
...
List pacmap_optimize_cpp(NumericMatrix Y_init,
  ...
  for (int itr = 0; itr < total; ++itr) {
    ...
    double loss = pacmap_grad_serial(Y, pair_nb, pair_MN, pair_FP, w_nb, w_mn, w_fp, grad);
    losses[itr] = loss;
...
List pacmap_fit_optimize_cpp(NumericMatrix Y_init,
  ...
  for (int itr = 0; itr < total; ++itr) {
    ...
    double loss = pacmap_grad_fit_serial(Y, pair_XP, w_nb, n_basis, grad);
    losses[itr] = loss;
    adam_step(Y, grad, m, v, beta1, beta2, lr, itr);
    if (verbose && ((itr + 1) % 25 == 0 || itr == 0))
